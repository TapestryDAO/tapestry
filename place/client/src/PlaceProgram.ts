...
export class PlaceProgram extends Program {
    ...
    public static parseInfoFromPurchaseGameplayTokenIx(
    ...
    static async findPatchPda(xPatch: number, yPatch: number): Promise<PublicKey> {
        ...
        let seeds = Buffer.concat([Buffer.from(this.PATCH_PDA_PREFIX), xBuf, yBuf]);
    ...
    static async findPlaceStatePda(): Promise<PublicKey> {
        let seeds = Buffer.concat([Buffer.from(this.PLACE_STATE_PDA_PREFIX)]);
    ...
    static async findGameplayTokenMintAta(
